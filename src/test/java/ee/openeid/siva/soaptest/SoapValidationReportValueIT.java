...
public class SoapValidationReportValueIT extends SiVaSoapTests {
     ...
     */
    @Test
    public void SoapDdocOptionalWarningElementIsPresent() {
        setTestFilesDirectory("ddoc/live/timemark/");
        post(validationRequestForDocument("SK-XML1.0.ddoc"))
                .then().rootPath(SOAP_VALIDATION_CONCLUSION_PREFIX)
                .body("ValidationWarnings.ValidationWarning[0].Content", Matchers.is(Constants.TEST_ENV_VALIDATION_WARNING))
                .body("ValidationWarnings.ValidationWarning[1].Content", Matchers.is("The algorithm SHA1 used in DDOC is no longer considered reliable for signature creation!"));
    }
